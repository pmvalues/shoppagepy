**SHOPPAGE v6.0 — FINAL AMENDED MODEL**
**Product Intelligence, Local Commerce, Media & Platform · Binding Business + Architecture Master**
Document status: v6.0 is the final strategic, operating and architecture constitution. It incorporates the v4.2 doctrine, v5.0 platform expansion and v6.0 adversarial execution amendments. Launch sequencing and feature gates are included because they are now binding controls. Monetisation remains dormant until evidence gates pass. Country-scoping is native throughout: Zimbabwe is the first jurisdiction instance; the South Africa formal/semi-formal instance is configuration, data and rights — not rearchitecture.

---


## AMENDMENT REGISTER — v4.2 → v6.0

This final version consolidates the original v4.2 doctrine, the v5.0 strategic expansion, and the v6.0 adversarial execution review.

### Binding changes

1. **Strategic end-state:** Shoppage is a product-intelligence, local-commerce, product-media and extension platform.
2. **Launch operating model:** the first commercial wedge is a demand-first local product-intent action engine; the broad platform is the destination, not the launch scope.
3. **Requests/Needs become first-class liquidity objects** alongside products and offers.
4. **The Referral Ledger becomes the Action Ledger**, measuring the full path from intent to merchant response and outcome evidence.
5. **Merchant capability is distinct from live inventory:** a merchant can be matched on capability even when stock/price requires confirmation.
6. **Markets become operating cells** where demand, merchants, freshness, evidence and SLAs are managed. Markets-in-Markets remain a core graph primitive, but deep topology is demand-pulled.
7. **Shorts and Shows remain first-class strategic surfaces**, but early Shorts are proof media; entertainment-first feeds are gated until supply, rights and moderation are mature.
8. **Grok/xAI is a governed AI adapter** for web/X intelligence, research, query interpretation, trend candidates and content assistance. It is never a commercial truth authority. Current xAI documentation confirms Web Search, X Search, custom function calling and structured outputs/tool use. citeturn0search0turn0search1turn0search2turn0search8
9. **Open platform strategy is staged:** internal contracts first, agencies/partners next, developer beta later, public ecosystem only after extraction/privacy controls pass.
10. **Payload 3 + Next.js shell + plain-TypeScript kernel remains the architectural baseline**, with one Drizzle/Postgres migration authority and public contracts isolated from Payload internals.
11. **No checkout/order/payment/settlement/fulfilment custody is added to the core.**
12. **Reference companies are learning references only**; the AI coder must not import their foreign transaction, ranking or identity assumptions.

### The governing distinction

> **v6.0 is simultaneously the strategic end-state and a staged execution constitution. The full platform exists in the architecture; only the evidence-backed subset is enabled at each gate.**


# EXECUTIVE DECISION — FINAL MODEL

## 1. Final verdict

**Proceed with Shoppage v6.0.** Use the full model below as the binding strategic and architectural constitution, but **do not build or launch every surface simultaneously**. The launch system is a narrow, high-intent, evidence-native demand-to-action engine that grows into the larger discovery network.

The model therefore has two layers:

```text
STRATEGIC END-STATE
Google-like product intent
+ Facebook-Groups-like commerce communities
+ YouTube-like product media
+ Product / Offer / Market / Demand intelligence
+ AI intelligence
+ Referral / Action network
+ Developer / agency platform

                         ↓

LAUNCH OPERATING MODEL
Demand
→ Fresh / capable local supply
→ Merchant response
→ Verified action
→ Outcome evidence
→ Repeat demand
...
EXPANSION
Markets → Proof Media → Community → Shows → Partner federation → Developer ecosystem
```

## 2. The category Shoppage is actually attacking

Shoppage is **not** trying to replace Google, Facebook or YouTube as general-purpose platforms. It is trying to replace their fragmented **commerce discovery behaviours** where they are weak:

- Google: product-intent research that lacks reliable local commercial truth.
- Facebook Groups: local commerce questions and recommendations that are unstructured, difficult to search and difficult to verify.
- YouTube/short-form video: product discovery that is not natively connected to current local offers, markets and actions.

The strategic wedge is therefore:

> **Find, verify, compare and contact the right local supplier faster than searching Google, asking Facebook, or scrolling WhatsApp.**

## 3. The primary economic loop

The unit of value is not a listing. It is a **resolved qualified need**.
...
Product + capability resolution
   ↓
Fresh / confirmable supply
   ↓
Merchant response
   ↓
WhatsApp / call / directions / quote / destination
   ↓
Outcome evidence
   ↓
Resolved or unresolved
   ↓
Learning + better matching
   ↓
More demand
```

Products remain foundational because they give the system canonical identity. **Demand determines where operational depth is created.**

## 4. What is core from day one vs gated

| Capability | Status | Rule |
|---|---|---|
| Product graph | Core | Global reference + country/local evidence separation |
| Offer graph | Core | Only evidence-backed commercial claims |
| Requests/Needs | Core | Primary liquidity mechanism |
| Merchant response | Core | Response quality becomes ranking/trust signal |
| Action Ledger | Core | Full action chain, not clicks alone |
| Search | Core | Product-intent answer engine |
| Markets | Core | Shallow everywhere; deep only where demand warrants |
| WhatsApp action rail | Core | External communication, not identity |
| Proof Shorts | Core| Canonical-linked, rights-cleared, evidence-oriented |
| Entertainment Shorts feed | Core | Requires supply, moderation and measurement |
| Shows |Core | Requires editorial/rights/disclosure controls |
| Public developer API | Core| Internal contracts first |
| Developer marketplace | Later | Only after anti-extraction controls |
| Sponsored ranking | Dormant | Separate candidate generator and disclosure |
| Native checkout | Excluded | Separate future product if ever required |

## 5. Launch KPI hierarchy

North-star metric:

> **Resolved Qualified Need Rate (RQNR)** = qualified buyer needs that reach a verified or credibly evidenced useful outcome ÷ qualified buyer needs.

Supporting metrics:

- qualified requests per week;
- merchant response rate;
- median response time;
- fresh-offer coverage in the active wedge;
- match acceptance rate;
- WhatsApp/call/directions/quote action rate;
- resolved action rate;
- repeat demand / saved-need rate;
- buyer-reported accuracy;
- stale-supply rate;
- complaint rate;
- local-availability hallucination rate;
- rights violation rate;
- cost per resolved qualified need.

Guardrails are mandatory: scam rate, privacy incidents, false freshness, wrong destination, AI hallucination, merchant coercion, partner leakage and ranking contamination.

## 6. Launch geography and category doctrine

Zimbabwe remains the first jurisdiction. **The launch unit is a corridor × category cell**, not “Zimbabwe” as a whole.

Select cells where:

- buyer intent is frequent/high-value;
- merchant density exists;
- WhatsApp/contact behaviour is strong;
- current supply can be refreshed;
- market/operator relationships are achievable;
- demand can be measured.

The existing 25,210-node Zimbabwe place/market scaffold is useful infrastructure, but it is not proof of verified operating markets; the implementation report explicitly identifies the hierarchy as discovery scaffolding requiring reconciliation and review. fileciteturn10file10

## 7. 100x principle

Win by producing the most reliable resolved commerce actions per unit of operational effort.**

The broader graph, media and ecosystem then compound from that evidence.
...
The target is not to clone any reference company but clone the relevant sections. Reference companies are bounded learning sources for the AI builder; Shoppage's domain model remains authoritative.
...
The Frontstore may merchandise products, offers, markets, creators and content, but every commercial claim remains governed by provenance and freshness. The frontstore should be exceptionally good to attract customers and be viral.
...
### 78. Search as a Google replacement for product intent
...
### 80. Shorts are first-class discovery objects
...
### 83. Watch → understand → compare → act
...
### 88. Grok is not the source of truth
...
### 90. AI content generation controls
...
### 102. Destination-first monetisation
...
# PART XXII — AI-CODER CONTRACT

### 108. Architecture rules
...
### 109. Forbidden implementation shortcuts
...
# PART XXIII — DOMAIN MAP

### 112. Core domain modules
...
# PART XXIV — COMPETITIVE REFERENCE MODEL

### 114. Reference companies
...
# PART XXVI — FINAL STRATEGIC MODEL

### 118. What Shoppage is
...
### 120. What Shoppage does not own
...
# FINAL CONSTITUTIONAL DECISION — SHOPPAGE v6.0

## A. Binding model

> **Shoppage is the discovery and intelligence layer between people and commerce.** It makes product search, local markets, community demand, product media and merchant destinations interoperable through one evidence-governed graph, while deliberately leaving transaction custody with the destination.
...
Market / Community Graph
        +
Demand / Request Graph
        +
Media Graph
        +
Trust / Rights Graph
        +
Action / Referral Graph
        +
AI Evidence Layer
        +
Developer Ecosystem
...
The moat is the **compounding evidence graph and operating loop**, not the UI and not a scraped directory.

## C. The strategic end-state

When the gates have been passed, the Frontstore can legitimately combine:

- product search and comparison;
- hyperlocal Markets-in-Markets;
- commerce communities;
- Shorts;
- Shows;
- product guides;
- requests and demand signals;
- merchant discovery;
- current offers;
- AI-assisted research and discovery;
- referrals to external destinations;
- agency tooling;
- partner federation;
- developer extensions.

The user experience can therefore feel like **Google + Facebook Groups + YouTube, but only for products and commerce discovery**, with a materially stronger structured data layer underneath.

## D. The launch reality

The launch is deliberately smaller:

```text
1–2 corridors
×
3 high-intent categories
×
Demand / Request engine
×
Freshness operations
×
Merchant response
×
WhatsApp / call / directions / quote actions
×
Outcome evidence
```

The architecture supports the entire end-state, but feature flags and gates prevent premature activation.

## E. Final non-negotiables

1. No checkout, payment, order or settlement custody in core.
2. No reference product presented as local supply.
3. No AI-generated commercial truth without evidence and authorised publication.
4. No market hierarchy from proximity or model inference alone.
5. No rights-cleared status assumed from technical accessibility.
6. No payment or sponsorship influence on organic ranking.
7. No external platform becomes a primary identity dependency.
8. No public API before internal contract, privacy and extraction tests pass.
9. No entertainment-first media expansion before proof-media and moderation controls work.
10. No national operational expansion before corridor-level resolved demand is repeatable.
11. No extension can bypass the domain kernel or access raw moat tables.
12. No claim is stronger than its evidence, freshness and confidence state.

## F. Final decision

**SHOPPAGE v6.0 is approved as the final strategic model and architecture constitution.**

**Implementation doctrine:** build the narrow operating loop first; preserve the full platform architecture from day one; activate broader surfaces only through evidence gates.

**Primary success condition:** Shoppage must demonstrably resolve local product demand better than the user's current combination of Google search, Facebook Groups, WhatsApp conversations and informal supplier hunting in the selected launch cells.

If that is achieved repeatedly, the larger product-search, market/community, media and platform ecosystem has a credible path to compound. If it is not achieved, adding Shorts, Shows, APIs or AI features does not solve the underlying liquidity problem.


## Verification note

The current xAI capability references in this model were checked against official xAI documentation on 24 August 2026: Web Search, X Search, custom function calling/tool use, and structured outputs with tools are documented capabilities. citeturn0search0turn0search1turn0search2turn0search8
