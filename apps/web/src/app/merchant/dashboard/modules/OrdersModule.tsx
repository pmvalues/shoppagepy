  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
        ...
        {/* Status Filters */}
        <div style={{ display: 'flex', gap: '0.35rem', background: 'var(--color-surface-subtle)', padding: '0.25rem', borderRadius: '10px' }}>
          {[
            ...
            <button
              ...
              style={{
                ...
                borderRadius: '8px',
                border: 'none',
                background: statusFilter === tab.id ? 'var(--color-surface)' : 'transparent',
                color: statusFilter === tab.id ? 'var(--color-content)' : 'var(--color-content-muted)',
                fontSize: '0.78rem',
      ...
      <div style={{ background: 'var(--color-surface)', borderRadius: '16px', border: '1px solid var(--color-line)', overflow: 'hidden', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
        {loading ? (
          <div style={{ padding: '3rem', textAlign: 'center', color: 'var(--color-content-muted)' }}>
            Loading live proforma reservations...
          </div>
        ) : orders.length === 0 ? (
          <div style={{ padding: '3rem', textAlign: 'center', color: 'var(--color-content-muted)' }}>
            <div style={{ fontSize: '2rem', marginBottom: '0.5rem' }}>📄</div>
            <h4 style={{ color: 'var(--color-content)', margin: 0 }}>No orders found for this status</h4>
          ...
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.825rem' }}>
              ...
              <tbody>
                {orders.map((ord) => (
                  <tr key={ord.id} style={{ borderBottom: '1px solid var(--color-line-subtle)' }}>
                    <td style={{ padding: '1rem', fontWeight: 800, color: 'var(--color-brand-ink)' }}>
                      {ord.invoiceNumber}
                    </td>
                    <td style={{ padding: '1rem' }}>
                      <div style={{ fontWeight: 700, color: 'var(--color-content)' }}>{ord.buyerCompany || ord.buyerName}</div>
                    ...
                    <td style={{ padding: '1rem', fontWeight: 900, color: 'var(--color-content)' }}>
                      R {ord.totalInclVatZar?.toLocaleString('en-ZA')}
                    </td>
                    <td style={{ padding: '1rem' }}>
                      <span style={{ fontSize: '0.72rem', fontWeight: 700, color: 'var(--color-brand-ink)', background: 'var(--color-brand-50)', padding: '2px 6px', borderRadius: '4px' }}>
                        📍 Counter Pickup
                      </span>
                    </td>
                    <td style={{ padding: '1rem' }}>
