...
export default function CollapsibleLocalMap({ merchants }: { merchants: Merchant[] }) {
  ...
  return (
    <section className="card" style={{ background: 'var(--color-surface)', border: '1px solid var(--color-line)', padding: '1.5rem', marginBottom: '2.5rem', borderRadius: '12px' }}>
      {/* Header Bar with Toggle Chevron */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
        <div>
          <h3 style={{ fontSize: '1.25rem', fontWeight: 800, color: 'var(--color-content)', margin: 0 }}>
            Nearby places
          </h3>
          <p style={{ fontSize: '0.8rem', color: 'var(--color-content-secondary)', margin: '0.2rem 0 0 0' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          <button
            ...
            style={{
              ...
              alignItems: 'center',
              gap: '0.4rem',
              background: isMapOpen ? 'var(--color-brand-50)' : 'var(--color-surface-subtle)',
              color: isMapOpen ? 'var(--color-brand-700)' : 'var(--color-content-secondary)',
          ...
          <span style={{ fontSize: '1.1rem', color: 'var(--color-content-muted)', cursor: 'pointer' }}>⋮</span>
        </div>
      </div>
      ...
      {isMapOpen && (
        ...
        >
          ...
          <svg width="100%" height="100%" viewBox="0 0 800 280" preserveAspectRatio="xMidYMid slice">
            ...
            <circle cx="400" cy="140" r="90" fill="none" stroke="rgba(5, 150, 105,0.15)" strokeWidth="40" />
          ...
          <div style={{ position: 'absolute', bottom: 12, right: 12, display: 'flex', flexDirection: 'column', gap: '4px' }}>
            <button style={{ width: 28, height: 28, background: 'var(--color-surface)', border: '1px solid var(--color-line-strong)', borderRadius: 4, fontWeight: 'bold' }}>+</button>
            <button style={{ width: 28, height: 28, background: 'var(--color-surface)', border: '1px solid var(--color-line-strong)', borderRadius: 4, fontWeight: 'bold' }}>-</button>
          </div>
          <div style={{ position: 'absolute', bottom: 12, left: 12, background: 'rgba(255,255,255,0.9)', padding: '0.25rem 0.6rem', borderRadius: '4px', fontSize: '0.7rem', color: 'var(--color-content-secondary)' }}>
            Open in Maps
          </div>
        </div>
      )}
        ...
        {localStores.map((merchant, idx) => (
          <div
            ...
            style={{
              ...
              borderBottom: idx !== localStores.length - 1 ? '1px solid var(--color-line-subtle)' : 'none',
              flexWrap: 'wrap',
              gap: '1rem',
            }}
          >
            <div>
              <h4 style={{ fontSize: '1.05rem', fontWeight: 800, margin: '0 0 0.25rem 0', color: 'var(--color-brand-700)' }}>
                <Link href={`/m/${merchant.id}`} style={{ color: 'inherit', textDecoration: 'none' }}>
                  {merchant.name}
                </Link>
              </h4>

              <div style={{ fontSize: '0.85rem', color: 'var(--color-content-secondary)', display: 'flex', alignItems: 'center', gap: '0.4rem', marginBottom: '0.25rem' }}>
                <span style={{ fontWeight: 700, color: 'var(--color-warning-ink)' }}>5,0 ★★★★★ ({merchant.googleReviewsCount || 78})</span>
                <span>·</span>
                <span>Solar energy equipment supplier</span>
              </div>

              <div style={{ fontSize: '0.8rem', color: 'var(--color-content-secondary)', marginBottom: '0.35rem' }}>
                📍 {merchant.addressText} · 📞 {merchant.contacts?.telephone || '068 168 7431'}
              </div>

              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.78rem' }}>
                <span style={{ color: 'var(--color-brand-700)', fontWeight: 700 }}>Open 24 hours</span>
                <span>·</span>
                <span style={{ color: 'var(--color-content-muted)', fontStyle: 'italic' }}>&quot;Outstanding customer service and a seamless installation.&quot;</span>
              </div>
            </div>
