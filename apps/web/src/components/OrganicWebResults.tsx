...
export default function OrganicWebResults({
  ...
  if (matchedDiscoveredOffers && matchedDiscoveredOffers.length > 0) {
    ...
    // Fallback to top verified national offers from SQLite database
    const allDbOffers = DiscoveredOffersStore.getAllDiscoveredOffers();
    allDbOffers.slice(0, 3).forEach((disc) => {
      const isSolar = disc.masterProductRef.includes('solar') || disc.masterProductRef.includes('deye') || disc.masterProductRef.includes('sunsynk') || disc.masterProductRef.includes('victron') || disc.masterProductRef.includes('dyness');
      const isHard = disc.masterProductRef.includes('cement') || disc.masterProductRef.includes('ppc') || disc.masterProductRef.includes('surebuild');
      const isTech = disc.masterProductRef.includes('samsung') || disc.masterProductRef.includes('iphone') || disc.masterProductRef.includes('a16');
      const emoji = isSolar ? '⚡' : isHard ? '🧱' : isTech ? '📱' : '🛒';
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem', marginBottom: '3rem' }}>
      {organicResults.map((res, i) => (
        <div key={i} style={{ display: 'flex', justifyContent: 'space-between', gap: '1.5rem', alignItems: 'flex-start' }}>
          <div style={{ flex: 1 }}>
            {/* Domain Breadcrumb */}
            <div style={{ fontSize: '0.8rem', color: 'var(--color-content)', display: 'flex', alignItems: 'center', gap: '0.35rem', marginBottom: '0.2rem' }}>
              <span style={{ width: '16px', height: '16px', borderRadius: '50%', background: 'var(--color-surface-subtle)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.7rem' }}>🌐</span>
              <span style={{ color: 'var(--color-content)' }}>{res.domain}</span>
              <span style={{ color: 'var(--color-content-muted)' }}>⋮</span>
            </div>

            {/* Clickable Blue Link Title */}
            <h3 style={{ fontSize: '1.25rem', fontWeight: 600, color: 'var(--color-brand-700)', margin: '0 0 0.35rem 0', lineHeight: 1.3 }}>
              {res.isInternal ? (
                <Link href={res.url} style={{ color: 'var(--color-brand-700)', textDecoration: 'none' }} className="hover-underline">
                  {res.title}
                </Link>
              ) : (
                <a href={res.url} target="_blank" rel="noopener noreferrer" style={{ color: 'var(--color-brand-700)', textDecoration: 'none' }} className="hover-underline">
                  {res.title}
                </a>
              )}
            </h3>
            ...
            {/* Snippet Description */}
            <p style={{ fontSize: '0.875rem', color: 'var(--color-content-secondary)', lineHeight: 1.55, margin: 0 }}>
              {res.date && <span style={{ color: 'var(--color-content-muted)' }}>{res.date} — </span>}
              {res.snippet}
            </p>
          </div>
