...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # Re-link merchants to the full roster of 3,296 markets
        self.stdout.write(self.style.NOTICE("==> Distributing Spatial Links across all 3,296 Malls & 3.1M Merchants..."))
        cur.execute("SELECT id FROM markets_market")
        all_market_ids = [r[0] for r in cur.fetchall()]

        self.stdout.write(self.style.SUCCESS(f"[OK] Total Indexed Spatial Commerce Nodes in Grid: {len(all_market_ids):,}"))
